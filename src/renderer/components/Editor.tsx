...
export const Editor: FunctionComponent<EditorProps> = (props: EditorProps) => {
    ...
    const editorDidMount = (editorInstance: monaco.editor.IStandaloneCodeEditor): void => {
        setEditor(editorInstance);
        setVersion(editorInstance.getModel().getAlternativeVersionId());
    };

    useEffect(handleSelectedWordChanges(editor, props.onWordSelected), [editor, props.onWordSelected]);
    useEffect(handleTextReplacements(props.textReplacements, editor), [editor, props.textReplacements]);
...
function handleSelectedWordChanges(
    ...
    return () => {
        ...
        const cursorChanges: Observable<WordAtPosition> =
            merge(
                fromEventPattern((handler: NodeEventHandler) => editor.onDidChangeCursorPosition(handler)),
                cursorUpdateKicker)
                .pipe(
                    map((): WordAtPosition => {
                        const cursorPosition: monaco.IPosition = editor.getPosition();
                        const wordAndColumns: monaco.editor.IWordAtPosition | null = editor.getModel()
        ...
        const selectionChanges: Observable<WordAtPosition> =
            fromEventPattern((handler: NodeEventHandler) => editor.onDidChangeCursorSelection(handler))
                .pipe(
                    map(() => {
                        const selectionRange: monaco.IRange = editor.getSelection();

                        return {
                            word: editor.getModel()
...
function handleTextReplacements(
    ...
    return () => {
        ...
        const subscription = textReplacements
            .subscribe((replacement: TextReplacement): void => {
                editor.focus();
...
function handleEditorEvents(
    ...
    return () => {
        ...
        const onFileOpened = (_: any, error: any, fileName: string, fileContents: string) => {
            if (!error) {
        ...
        const onReplace = () => editor.trigger('', 'editor.action.startFindReplaceAction', '');
        ipcRenderer.on(IpcChannels.REPLACE, onReplace);

        return () => {
            ipcRenderer.removeListener(IpcChannels.FILE_SAVE_ENDED, onFileSaveEnded);
            ipcRenderer.removeListener(IpcChannels.ATTEMPT_NEW_FILE, onNewFileAttempt);
            ipcRenderer.removeListener(IpcChannels.ATTEMPT_QUIT, onQuitAttempt);
            ipcRenderer.removeListener(IpcChannels.NEW_FILE_CREATED, onNewFileCreated);
            ipcRenderer.removeListener(IpcChannels.FILE_OPENED, onFileOpened);
            ipcRenderer.removeListener(IpcChannels.REQUEST_EDITOR_TEXT, onTextRequested);
